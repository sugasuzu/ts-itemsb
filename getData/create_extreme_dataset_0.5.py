...
class ExtremeDatasetCreator:
    ...
    def _create_readme(self, output_dir):
        ...
        with open(readme_path, 'w', encoding='utf-8') as f:
            ...
            f.write("- **Excluded**: -0.5% < X < 0.5% (normal fluctuations near zero)\n\n")

            f.write("### File Structure\n\n")
            f.write("```\n")
            f.write("forex_data/extreme_gnminer/\n")
            f.write("├── USDJPY_positive.txt   # Positive extremes (X >= 0.5%)\n")
            f.write("├── USDJPY_negative.txt   # Negative extremes (X <= -0.5%)\n")
...
Examples:
  ...
  python3 getData/create_extreme_dataset.py --threshold 0.5
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help='Extreme value threshold in percentage (default: 0.5)'
    )
    parser.add_argument(
        '--pairs',
        nargs='+',
