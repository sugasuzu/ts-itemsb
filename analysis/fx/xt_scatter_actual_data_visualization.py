#!/usr/bin/env python3
"""
ルールベースX-T散布図作成スクリプト（全体データ版）

全体データと各ルールの局所分布を重ねて可視化し、
ルール適用による局所化効果を示します。

使用方法:
    python analysis/fx/xt_scatter_actual_data_visualization.py
"""
...
class ActualDataScatterPlotterXT:
    ...
    def __init__(self, pair):
        ...
        """
        self.pair = pair
        self.base_dir = Path("analysis/fx/visualizations_xt_actual")
        self.output_dir = self.base_dir / pair
        self.output_dir.mkdir(parents=True, exist_ok=True)

        # ファイルパス（directionなし）
        self.rule_file = Path(f"output/{pair}/pool/zrp01a.txt")
        self.full_data_file = Path(f"forex_data/gnminer_individual/{pair}.txt")

        print(f"\n{'='*80}")
        print(f"Rule-Based X-T Scatter Plot Generator (Full Dataset)")
        print(f"{'='*80}")
        print(f"Pair:          {pair}")
...
def main():
    """メイン処理 - 全通貨ペア・全ルールを自動処理"""
    # スクリプトの位置から base_dir を特定
    script_dir = Path(__file__).resolve().parent  # analysis/fx/
    ...
    if output_dir.exists():
        for pair_dir in output_dir.iterdir():
            if pair_dir.is_dir() and (pair_dir / "pool/zrp01a.txt").exists():
                forex_pairs.append(pair_dir.name)

    if not forex_pairs:
        print("Error: No forex pairs found in output directory")
        return

    forex_pairs.sort()  # アルファベット順にソート

    print(f"\n{'#'*70}")
    print(f"# Auto-detected {len(forex_pairs)} forex pairs:")
    print(f"# {', '.join(forex_pairs)}")
    print(f"{'#'*70}\n")
    ...
    for forex_pair in forex_pairs:
        ...
        try:
            plotter = ActualDataScatterPlotterXT(forex_pair)
    ...
    print(f"  Processed {len(forex_pairs)} forex pairs")
    print(f"{'='*70}\n")
