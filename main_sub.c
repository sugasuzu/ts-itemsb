...
/* ルールマイニング制約 - 2段階Q1方式（バランス型・多様性重視） */
#define Q1_THRESHOLD 0.10     // 0.10% Q1判定閾値
#define MIN_Q1_PRECISION 0.32 // 32% Q1適合率（やや緩和・多様性確保）

/* Phase 2: Q1データでの高品質フィルタ（バランス調整） */
#define Minsup 0.0022           // 0.22% サポート率（やや緩和）
#define Maxsigx 0.45            // 最大標準偏差 0.45%（やや緩和）
#define MIN_SUPPORT_COUNT 5     // 統計的信頼性（最低5回・緩和）
#define MIN_MEAN_THRESHOLD 0.07 // 0.07% 最小平均値（やや緩和）

#define MIN_ATTRIBUTES 2 // 最小属性数 2以上（複雑なパターンを要求）

/* 旧システムとの互換性（未使用） */
#define QUADRANT_THRESHOLD 0.5 // 0.5% 象限判定閾値（2段階方式では未使用）
...
struct temporal_rule
{
    ...
    /* 検証結果（Phase 3: 全データ検証） */
    int validation_total_matches;   // 全データでのマッチ数
    int validation_q1_matches;      // そのうちQ1に該当する数
    double validation_q1_precision; // Q1適合率
    double validation_full_support; // 全データでのサポート率
    int validation_accepted;        // 採用フラグ (1=採用, 0=不採用)
};
...
/* 全データ保存用（Phase 1: 検証用） */
int Nrd_full = 0;
int **data_buffer_full = NULL;
double *x_buffer_full = NULL;
char **timestamp_buffer_full = NULL;
int *q1_to_full_index = NULL; // マッピング: Q1インデックス → 全データインデックス
                       ...
                       double *future_min_array, double *future_max_array, int matched_count)
{
    ...
    // 象限判定（2段階アプローチでは不要 - Q1データのみで学習）
    quadrant = 1; // Q1として扱う（全データがQ1なので常にパス）
    ...
    */
    ...
    /* Stage 3: 平均値チェック（Q1データ内でも高リターン） */
    for (i = 0; i < FUTURE_SPAN; i++)
    {
        if (future_mean_array[i] < MIN_MEAN_THRESHOLD)
        {
            filter_rejected_by_minsup++; // 既存カウンタを流用
            if (should_debug)
            {
                fprintf(stderr, "[FILTER] Rejected by MinMean: mean[t+%d]=%.4f%% < MinMean=%.4f%%\n",
                        i + 1, future_mean_array[i], MIN_MEAN_THRESHOLD);
                debug_sample_count++;
            }
            return 0; // 低リターン → 除外
        }
...
/* ===== Phase 1: Q1データフィルタリング ===== */
int filter_q1_data_from_full()
{
    int q1_count = 0;
    int i;

    printf("\n========== Phase 1: Q1 Data Filtering ==========\n");
    printf("Q1 Threshold: %.2f%% (both t+1 and t+2 must be >= this value)\n", Q1_THRESHOLD);

    // Q1データ数をカウント
    for (i = 0; i < Nrd_full - FUTURE_SPAN; i++)
    {
        double x_t1 = x_buffer_full[i + 1];
        double x_t2 = x_buffer_full[i + 2];

        if (x_t1 >= Q1_THRESHOLD && x_t2 >= Q1_THRESHOLD)
        {
            q1_count++;
        }
    }

    printf("Full Dataset: %d points\n", Nrd_full);
    printf("Q1 Dataset: %d points (%.1f%%)\n", q1_count, 100.0 * q1_count / Nrd_full);

    if (q1_count == 0)
    {
        printf("ERROR: No Q1 data points found! Cannot proceed.\n");
        return -1;
    }
    ...
    // マッピング配列を割り当て
    q1_to_full_index = (int *)malloc(Nrd * sizeof(int));
    if (q1_to_full_index == NULL)
    {
        printf("ERROR: Failed to allocate q1_to_full_index\n");
        return -1;
    }
    ...
    for (i = 0; i < Nrd_full - FUTURE_SPAN; i++)
    {
        ...
        {
            // データをコピー
            memcpy(data_buffer[q1_idx], data_buffer_full[i], Nzk * sizeof(int));
            x_buffer[q1_idx] = x_buffer_full[i];
            strncpy(timestamp_buffer[q1_idx], timestamp_buffer_full[i], MAX_ATTR_NAME - 1);
            timestamp_buffer[q1_idx][MAX_ATTR_NAME - 1] = '\0';

            q1_to_full_index[q1_idx] = i; // マッピングを記録

            q1_idx++;
        }
    }

    printf("Q1 data filtering complete: %d points extracted\n", q1_idx);
    printf("================================================\n\n");
...
/* ===== Phase 3: 全データでの検証 ===== */
void validate_rules_on_full_data(int rule_count)
{
    int rule_idx, i, attr_idx;

    printf("\n========== Phase 3: Validation on Full Data ==========\n");
    printf("Full Dataset: %d points\n", Nrd_full);
    printf("Acceptance Criteria: Q1 Precision >= %.0f%%\n", MIN_Q1_PRECISION * 100.0);
    printf("======================================================\n\n");
    ...
    {
        struct temporal_rule *rule = &rule_pool[rule_idx];
        int total_match = 0;
        int q1_match = 0;
        ...
        for (i = 0; i < Nrd_full - FUTURE_SPAN; i++)
        {
            ...
            // ルールの各属性をチェック
            for (attr_idx = 0; attr_idx < MAX_ATTRIBUTES; attr_idx++)
            {
                int attr_id = rule->antecedent_attrs[attr_idx];
                if (attr_id == 0)
                    break; // 属性終了

                int delay = rule->time_delays[attr_idx];
                int data_idx = i - delay;

                if (data_idx < 0 || data_buffer_full[data_idx][attr_id - 1] != 1)
                {
                    matched = 0;
                    break;
                }
        ...
        // 検証結果を計算
        rule->validation_total_matches = total_match;
        rule->validation_q1_matches = q1_match;
        rule->validation_q1_precision = (total_match > 0) ? (double)q1_match / total_match : 0.0;
        rule->validation_full_support = (double)total_match / Nrd_full;
        rule->validation_accepted = (rule->validation_q1_precision >= MIN_Q1_PRECISION);

        if (rule->validation_accepted)
        {
            accepted_count++;
        }

        // 結果を出力
        printf("Rule #%d: Precision=%.1f%% (%d/%d), FullSupport=%.2f%% - %s\n",
               rule_idx + 1,
               rule->validation_q1_precision * 100.0,
               q1_match, total_match,
               rule->validation_full_support * 100.0,
               rule->validation_accepted ? "✓ ACCEPTED" : "✗ REJECTED");
    }

    printf("\n======================================================\n");
    printf("Validation Summary:\n");
    printf("  Total Rules: %d\n", rule_count);
    printf("  Accepted: %d (%.1f%%)\n", accepted_count, 100.0 * accepted_count / rule_count);
    printf("  Rejected: %d (%.1f%%)\n", rule_count - accepted_count, 100.0 * (rule_count - accepted_count) / rule_count);
    printf("======================================================\n\n");
}
...
void setup_paths_for_stock(const char *code)
{
    ...
    // データタイプを判定（BTC, ETH等 → crypto, それ以外 → forex）
    const char *data_dir;
    if (strcmp(stock_code, "BTC") == 0 || strcmp(stock_code, "ETH") == 0 ||
        strcmp(stock_code, "BNB") == 0 || strcmp(stock_code, "XRP") == 0)
    {
        data_dir = "1-deta-enginnering/crypto_data_hourly";
    }
    else
    {
        data_dir = "1-deta-enginnering/forex_data_daily";
    }

    // データファイルパスを設定
    snprintf(data_file_path, sizeof(data_file_path),
             "%s/%s.txt", data_dir, stock_code);

    // 出力ベースディレクトリを設定
    snprintf(output_base_dir, sizeof(output_base_dir),
             "%s/output/%s", data_dir, stock_code);
...
int process_single_stock(const char *code)
{
    ...
    /* ===== Phase 1: 全データ保存とQ1フィルタリング ===== */

    // 全データを保存
    Nrd_full = Nrd;
    data_buffer_full = data_buffer;
    x_buffer_full = x_buffer;
    timestamp_buffer_full = timestamp_buffer;

    // Q1データのみを抽出
    printf("========== Phase 2: GNP on Q1 Data ==========\n");
    printf("Starting GNP evolution on Q1-filtered dataset...\n");
    printf("=============================================\n\n");

    int q1_result = filter_q1_data_from_full();
    if (q1_result < 0)
    {
        printf("ERROR: Q1 filtering failed\n");
        return 1;
    }
    ...
    // rule_poolのルールを全データで検証
    if (state.rule_count > 1)
    {
        validate_rules_on_full_data(state.rule_count - 1);
    }
    else
    {
        printf("\nNo rules discovered to validate.\n");
    }
