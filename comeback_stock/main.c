...
/* ファイル名 */
#define DATANAME "../nikkei225_data/gnminer_individual/3436.txt"
...
void setup_paths_for_stock(const char *code)
{
    ...
    // 出力ベースディレクトリを設定（comeback_stock配下に出力）
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1 && (strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "--help") == 0))
    {
        ...
        printf("Arguments:\n");
        printf("  <stock_code>  Nikkei 225 stock code (e.g., 3436, 4755, 9984)\n\n");
        printf("Examples:\n");
        printf("  %s 3436  # High volatility stock (top 1)\n", argv[0]);
        printf("  %s 4755  # High volatility stock (top 2)\n", argv[0]);
        printf("  %s 9984  # SoftBank Group\n\n", argv[0]);
        printf("Note:\n");
        printf("  - Data files must exist: ../nikkei225_data/gnminer_individual/{CODE}.txt\n");
        printf("  - Results will be saved to: comeback_stock/output/{CODE}/\n");
        printf("  - Use Makefile: make test, make run\n\n");
        printf("==============================================\n");
        return 0;
    }
