...
#define MAX_TIME_DELAY 5 // 過去スパン： t-0(これはデフォルト), t-1, t-2, t-3, t-4, t-5 (Action 3: 拡大)
...
int filter_rejected_by_quadrant_rate = 0;      // MIN_CONCENTRATIONで却下（集中率不足）
int filter_rejected_by_quadrant_deviation = 0; // MAX_DEVIATIONで却下（支配象限からの逸脱）
...
static int determine_quadrant_by_rate_with_concentration(int *matched_indices, int match_count, double *concentration_out, int *in_quadrant_count_out)
{
    ...
    *concentration_out = concentration_rate;
    ...
    /* Step 4: 集中度チェック */
    if (concentration_rate < MIN_CONCENTRATION)
    {
        filter_rejected_by_quadrant_rate++;
        return 0;
    }

    /* Step 5: 支配象限からの逸脱チェック */
    for (i = 0; i < match_count; i++)
    {
        future_t1 = get_future_value(matched_indices[i], 1);
        future_t2 = get_future_value(matched_indices[i], 2);
        ...
        {
            if (future_t1 < -MAX_DEVIATION || future_t2 < -MAX_DEVIATION)
            {
                filter_rejected_by_quadrant_deviation++;
                return 0; // Q1支配なのに負の領域に逸脱
            }
        }
...
void create_trial_files(struct trial_state *state)
{
    ...
    if (file != NULL)
    {
        fprintf(file, "Generation\tRules\tHighSup\tLowVar\tAvgFitness\tMin1Attr\n");
        fclose(file);
    }
    else
    {
...
void write_filter_statistics_to_file()
{
    ...
    fprintf(fp, "MIN_CONCENTRATION:        %.2f\n", MIN_CONCENTRATION);
    fprintf(fp, "MAX_DEVIATION:            %.2f\n", MAX_DEVIATION);
    fprintf(fp, "========================================\n\n");

    // フィルタ統計
    int total_quadrant_rejected = filter_rejected_by_quadrant_rate + filter_rejected_by_quadrant_deviation;
    int total_evaluated = filter_rejected_by_minsup + total_quadrant_rejected + filter_passed_total;
    ...
    if (total_evaluated > 0)
    {
        ...
        if (total_quadrant_rejected > max_rejected)
        {
            max_rejected = total_quadrant_rejected;
            bottleneck = "Quadrant (Concentration & Deviation)";
        }
...
void print_final_statistics()
{
    ...
    int total_evaluated = filter_rejected_by_minsup +
                          total_quadrant_rejected_term + filter_passed_total;
    if (total_evaluated > 0)
    {
        printf("\n========== Filter Statistics ==========\n");
        ...
        printf("    - By MIN_CONCENTRATION: %6d (%.1f%%)\n",
               filter_rejected_by_quadrant_rate,
               100.0 * filter_rejected_by_quadrant_rate / total_evaluated);
        printf("    - By MAX_DEVIATION:     %6d (%.1f%%)\n",
               filter_rejected_by_quadrant_deviation,
               100.0 * filter_rejected_by_quadrant_deviation / total_evaluated);
