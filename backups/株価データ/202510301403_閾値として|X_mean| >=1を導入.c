...
void calculate_negative_counts()
{
    ...
    // 実際に評価される安全な範囲を取得
    get_safe_data_range(&safe_start, &safe_end);
    int effective_records = safe_end - safe_start;
    ...
    for (individual = 0; individual < Nkotai; individual++)
    {
        for (k = 0; k < Npn; k++)
        {
            for (i = 0; i < Nmx; i++)
            {
                // ネガティブカウント = 実際に評価される範囲のレコード数
                // サポート率 = マッチ数 / 評価範囲のレコード数（統計的に正しい定義）
                negative_count[individual][k][i] = effective_records;
            }
        }
    }
}
...
{
    return (fabs(x_mean) >= 1.0 &&             // X平均の絶対値が1.0以上（極値）
...
void extract_rules_from_individual(struct trial_state *state, int individual)
{
    ...
    for (k = 0; k < Npn; k++)
    {
        ...
        for (loop_j = MIN_ATTRIBUTES; loop_j < 9; loop_j++)
        {
            ...
            // ルールの品質チェック
            if (check_rule_quality(x_mean, sigma_x, support, j2))
            {
