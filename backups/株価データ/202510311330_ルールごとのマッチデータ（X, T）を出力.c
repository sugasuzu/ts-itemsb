...
struct temporal_rule
{
    ...
    // 時間間隔の規則性（Interval-based）
    double t_interval_mean;     // イベント間の平均間隔（日数）
    double t_interval_variance; // 時間間隔の分散（日数²）
    double t_interval_std;      // 時間間隔の標準偏差（日数） = 規則性指標
    int time_regularity_flag;   // 時間的規則性フラグ（1:規則的, 0:不規則）
...
int load_csv_with_header()
{
    ...
    printf("Total records: %d (pre-filtered, both directions)\n", Nrd);
    printf("Positive extreme count (X >= 1.0): %d\n", positive_count);
    printf("Negative extreme count (X <= -1.0): %d\n", negative_count);
    printf("Zero-range count (|X| < 1.0): %d (should be 0)\n", Nrd - positive_count - negative_count);
    printf("================================================\n\n");

    // 読み込み結果のサマリを表示
    printf("Data loaded successfully (BOTH DIRECTIONAL EXTREMES):\n");
    printf("  Records: %d\n", Nrd);
    printf("  Positive extremes: %d (%.1f%%)\n", positive_count, 100.0 * positive_count / Nrd);
    printf("  Negative extremes: %d (%.1f%%)\n", negative_count, 100.0 * negative_count / Nrd);
    printf("  Attributes: %d\n", Nzk);
...
void create_output_directories()
{
    ...
    // 親ディレクトリを含めて再帰的に作成
    snprintf(cmd, sizeof(cmd), "mkdir -p \"%s\" \"%s\" \"%s\" \"%s\" \"%s\"",
...
void initialize_rule_pool()
{
    ...
    for (i = 0; i < Nrulemax; i++)
    {
        ...
        // 時間間隔規則性の初期化
        rule_pool[i].t_interval_mean = 0.0;
        rule_pool[i].t_interval_variance = 0.0;
        rule_pool[i].t_interval_std = 0.0;
        rule_pool[i].time_regularity_flag = 0;
...
/**
 * 時間間隔の規則性を計算（Interval-based）
 * イベント発生間隔の標準偏差を計算し、規則性を定量化
 * @param rule 分析対象のルール
 * @param individual 個体ID
 * @param k 処理ノードID
 * @param depth 深さ
 */
void calculate_temporal_interval_regularity(struct temporal_rule *rule, int individual, int k, int depth)
{
    int i, idx;
    int count = matched_time_count[individual][k][depth];

    // マッチが2つ未満の場合は間隔が計算できない
    if (count < 2)
    {
        rule->t_interval_mean = 0.0;
        rule->t_interval_variance = 0.0;
        rule->t_interval_std = 0.0;
        rule->time_regularity_flag = 1; // 単一時点は最も規則的とみなす
        return;
    }
    ...
    double sum_intervals_sq = 0.0;

    for (i = 0; i < n_intervals; i++)
    {
        int current_idx = matched_time_indices[individual][k][depth][i];
        int next_idx = matched_time_indices[individual][k][depth][i + 1];

        if (current_idx < 0 || current_idx >= Nrd || next_idx < 0 || next_idx >= Nrd)
            continue;

        // 連続イベント間の間隔（日数）
        int interval = calculate_days_between(&time_info_array[current_idx],
                                              &time_info_array[next_idx]);

        sum_intervals += (double)interval;
        sum_intervals_sq += (double)(interval * interval);
    }

    // 平均間隔
    double mean_interval = sum_intervals / (double)n_intervals;

    // 標本分散: (Σx²/n) - (Σx/n)²
    double variance = (sum_intervals_sq / (double)n_intervals) - (mean_interval * mean_interval);
    ...
    // 不偏分散に変換: s² = n/(n-1) × σ²（n_intervals >= 2の場合）
    if (n_intervals >= 2)
    {
        variance = variance * n_intervals / (n_intervals - 1);
    }
    ...
    // ルールに設定
    rule->t_interval_mean = mean_interval;
    rule->t_interval_variance = variance;
    rule->t_interval_std = std_dev;

    // 時間的規則性フラグ（間隔の標準偏差が閾値以下なら規則的）
    rule->time_regularity_flag = (std_dev <= Maxsigt_interval) ? 1 : 0;
}
 ...
 * @param t_interval_std T間隔標準偏差（時間的規則性、負値の場合はチェックスキップ）
 * @return 品質基準を満たす場合1、満たさない場合0
 */
int check_rule_quality(double x_mean, double sigma_x, double support, int num_attributes,
                       double t_interval_std)
{
    // 基本的な品質チェック
    int basic_check = (
        // fabs(x_mean) >= MinXmean &&
        sigma_x <= Maxsigx &&              // X標準偏差が閾値以下
        support >= Minsup &&               // サポート値が閾値以上
        num_attributes >= MIN_ATTRIBUTES); // 最小属性数以上

    // t_interval_stdが有効な場合（>= 0）は時間的規則性もチェック
    if (t_interval_std >= 0.0)
    {
        basic_check = basic_check && (t_interval_std <= Maxsigt_interval);
    }
                       ...
                       int individual, int k, int depth)
{
    ...
    // t統計量の計算: t = X_mean / (X_sigma / sqrt(n))
    // これは X_mean がゼロと統計的に異なるかを検定する
    if (support_count > 1 && x_sigma > 0)
    {
        double standard_error = x_sigma / sqrt((double)support_count);
        rule_pool[idx].t_statistic = x_mean / standard_error;
    }
    else
    {
        rule_pool[idx].t_statistic = 0.0;
    }

    // 時間パターンを分析（月別、四半期別、曜日別の統計を計算）
    analyze_temporal_patterns(&rule_pool[idx], individual, k, depth);

    // 時間間隔の規則性を計算（t_interval_mean, t_interval_std, time_regularity_flagを計算）
    calculate_temporal_interval_regularity(&rule_pool[idx], individual, k, depth);

    // 時間的規則性チェック：t_interval_std > Maxsigt_intervalの場合はルールを棄却
    if (rule_pool[idx].t_interval_std > Maxsigt_interval)
    {
        // このルールは時間的に不規則すぎるため採用しない
        return;
    }
...
void extract_rules_from_individual(struct trial_state *state, int individual)
{
    ...
    for (k = 0; k < Npn; k++)
    {
        ...
        {
            // 現在の深さの統計を取得
            double x_mean = x_sum[individual][k][loop_j];
            ...
            // ルールの品質チェック（初期チェック、t_interval_stdは後で確認）
            if (check_rule_quality(x_mean, sigma_x, support, j2, -1.0))
            {
...
void create_trial_files(struct trial_state *state)
{
    ...
    {
        fprintf(file, "Attr1\tAttr2\tAttr3\tAttr4\tAttr5\tAttr6\tAttr7\tAttr8\t");
        fprintf(file, "X_mean\tX_sigma\tsupport_count\tsupport_rate\tt_statistic\tHighSup\tLowVar\tNumAttr\t");
...
void write_local_output(struct trial_state *state)
{
    ...
    for (i = 1; i < state->rule_count; i++)
    {
        ...
        // 予測値の統計
        fprintf(file, "%8.3f\t%5.3f\t%d\t%6.4f\t%7.3f\t%2d\t%2d\t",
                rule_pool[i].x_mean,
                rule_pool[i].x_sigma,
                rule_pool[i].support_count,
                rule_pool[i].support_rate,
                rule_pool[i].t_statistic,
...
void write_global_pool(struct trial_state *state)
{
    ...
    if (file_a != NULL)
    {
        ...
        fprintf(file_a, "X_mean\tX_sigma\tsupport_count\tsupport_rate\tt_statistic\tNegative\tHighSup\tLowVar\tNumAttr\t");
        fprintf(file_a, "T_interval_mean\tT_interval_std\tTimeSpan\t");
        ...
        for (i = 0; i < global_rule_count; i++)
        {
            ...
            // 予測値の統計
            fprintf(file_a, "%8.3f\t%5.3f\t%d\t%6.4f\t%7.3f\t%d\t%d\t%d\t",
            ...
            // 時間間隔の規則性
            fprintf(file_a, "%8.3f\t%7.3f\t%d\t",
                    global_rule_pool[i].t_interval_mean,
                    global_rule_pool[i].t_interval_std,
    ...
    if (file_b != NULL)
    {
        ...
        for (i = 0; i < global_rule_count; i++)
        {
            if (fabs(global_rule_pool[i].t_statistic) >= 2.0)
            {
                significant_count++;
            }
        }
        ...
        fprintf(file_b, "# Significant Rules (|t|>=2.0): %d (%.1f%%)\n",
                significant_count, 100.0 * significant_count / global_rule_count);
        fprintf(file_b, "\n");
        ...
        for (i = 0; i < display_limit; i++)
        {
            ...
            fprintf(file_b, "\n");
            fprintf(file_b, "   => Predicted_X: %.3f±%.3f (t=%.3f, n=%d)",
                    global_rule_pool[i].x_mean, global_rule_pool[i].x_sigma,
                    global_rule_pool[i].t_statistic, global_rule_pool[i].support_count);

            // 統計的有意性マーカー
            if (fabs(global_rule_pool[i].t_statistic) >= 2.0)
            {
                fprintf(file_b, " **SIGNIFICANT**");
            }
            fprintf(file_b, "\n");
            ...
            fprintf(file_b, "   Temporal: Month=%d, Quarter=Q%d, Day=%d, Span=%d days\n",
                    ...
                    global_rule_pool[i].dominant_day_of_week,
                    global_rule_pool[i].time_span_days);
            fprintf(file_b, "   Time Regularity: Interval_mean=%.3f days, Interval_std=%.3f days\n",
                    global_rule_pool[i].t_interval_mean,
                    global_rule_pool[i].t_interval_std);
            fprintf(file_b, "   Extreme Signal: Score=%.3f (p=%.4f)\n",
...
/**
 * 時間詳細情報を出力（各ルールの全マッチ日付リスト）
 * 各銘柄の専用ディレクトリに出力
 * @param state 試行状態（引数としては使用しないが互換性のため保持）
 */
void write_temporal_detail_file(struct trial_state *state)
{
    char temporal_file_path[256];
    snprintf(temporal_file_path, sizeof(temporal_file_path),
             "%s/pool/zrp01a_temporal.txt", output_base_dir);

    FILE *file = fopen(temporal_file_path, "w");
    if (file == NULL)
    {
        printf("警告: 時間詳細ファイルを作成できませんでした: %s\n", temporal_file_path);
        return;
    }

    fprintf(file, "================================================================================\n");
    fprintf(file, "Temporal Detail Report: All Matched Dates for Each Rule\n");
    fprintf(file, "================================================================================\n");
    fprintf(file, "Generated: %s\n", __DATE__);
    fprintf(file, "Total Rules: %d\n", global_rule_count);
    fprintf(file, "================================================================================\n\n");
    ...
    for (int i = 0; i < global_rule_count; i++)
    {
        ...
        fprintf(file, "  X_mean:           %8.3f%%\n", global_rule_pool[i].x_mean);
        fprintf(file, "  X_sigma:          %8.3f%%\n", global_rule_pool[i].x_sigma);
        fprintf(file, "  t_statistic:      %8.3f", global_rule_pool[i].t_statistic);
        if (fabs(global_rule_pool[i].t_statistic) >= 2.0)
        {
            fprintf(file, " **SIGNIFICANT**");
        }
        fprintf(file, "\n");
        fprintf(file, "  support_count:    %8d\n", global_rule_pool[i].support_count);
        fprintf(file, "  support_rate:     %8.4f\n", global_rule_pool[i].support_rate);
        fprintf(file, "\n");

        // 時間情報
        fprintf(file, "Time Information:\n");
        fprintf(file, "  First match:      %s\n", global_rule_pool[i].start_date);
        fprintf(file, "  Last match:       %s\n", global_rule_pool[i].end_date);
        fprintf(file, "  TimeSpan:         %d days (%.1f years)\n",
                global_rule_pool[i].time_span_days,
                global_rule_pool[i].time_span_days / 365.25);
        fprintf(file, "  T_interval_mean:  %8.3f days\n", global_rule_pool[i].t_interval_mean);
        fprintf(file, "  T_interval_std:   %8.3f days\n", global_rule_pool[i].t_interval_std);
        fprintf(file, "\n");

        // 全マッチ日付のリスト
        int match_count = global_rule_pool[i].matched_count_vis;
        fprintf(file, "All Matched Dates (%d matches):\n", match_count);

        if (match_count > 0 && global_rule_pool[i].matched_indices != NULL)
        {
            for (int m = 0; m < match_count && m < Nrd; m++)
            {
                int idx = global_rule_pool[i].matched_indices[m];
                if (idx >= 0 && idx < Nrd)
                {
                    fprintf(file, "  %3d. %s\n", m + 1, timestamp_buffer[idx]);
                }
            }
        }
        else
        {
            fprintf(file, "  (No match data available)\n");
        }
    ...
    fprintf(file, "================================================================================\n");
    fprintf(file, "End of Temporal Detail Report\n");
    fprintf(file, "================================================================================\n");

    fclose(file);

    printf("  ✓ Temporal detail file created: %s\n", temporal_file_path);
}

/**
 * ルールごとのマッチデータ（X, T）を出力
 * 各ルールがどのデータポイントとマッチしたかを詳細に記録
 * @param state 試行状態
 */
void write_matched_data_file(struct trial_state *state)
{
    char matched_file_path[256];
    snprintf(matched_file_path, sizeof(matched_file_path),
             "%s/pool/rule_matched_data.txt", output_base_dir);

    FILE *file = fopen(matched_file_path, "w");
    if (file == NULL)
    {
        fprintf(stderr, "Error: Cannot create matched data file: %s\n", matched_file_path);
        return;
    }

    fprintf(file, "Rule-wise Matched Data Records (X, T)\n");
    fprintf(file, "================================================================================\n");
    fprintf(file, "This file shows all data points (X, T) that matched each rule.\n");
    fprintf(file, "Format: Rule definition followed by list of matched (X, T) pairs.\n");
    fprintf(file, "================================================================================\n\n");
    ...
    for (int i = 0; i < global_rule_count; i++)
    {
        ...
        int first_attr = 1;
        for (int j = 0; j < MAX_ATTRIBUTES && rule->antecedent_attrs[j] != 0; j++)
        {
            int attr_id = rule->antecedent_attrs[j];
            int delay = rule->time_delays[j];

            if (!first_attr)
                fprintf(file, " AND ");
            fprintf(file, "%s(t-%d)", attribute_dictionary[attr_id - 1], delay);
            first_attr = 0;
        }
        fprintf(file, "\n");

        // 統計情報
        fprintf(file, "THEN X at t+1: mean=%.3f, sigma=%.3f\n",
                rule->x_mean, rule->x_sigma);
        fprintf(file, "Support: %d matches (%.2f%%)\n",
                rule->support_count, rule->support_rate * 100);
        fprintf(file, "Time span: %s to %s (%d days)\n",
                rule->start_date, rule->end_date, rule->time_span_days);
        fprintf(file, "\n");

        // マッチしたデータポイントを出力
        fprintf(file, "Matched Data Points (%d records):\n", rule->support_count);
        fprintf(file, "  No.  |     T (Date)    |    X (%%)\n");
        fprintf(file, "-------+-----------------+----------\n");

        for (int j = 0; j < rule->support_count && j < Nrd; j++)
        {
            int idx = rule->matched_indices[j];
            if (idx >= 0 && idx < Nrd)
            {
                fprintf(file, "%6d | %15s | %8.3f\n",
                        j + 1, timestamp_buffer[idx], x_buffer[idx]);
            }
        }
    ...
    fclose(file);
    printf("  ✓ Matched data file created: %s\n", matched_file_path);
}
...
int process_single_stock(const char *code)
{
    ...
    // 時間詳細情報を出力（全マッチ日付リスト）
    printf("\n========== Writing Temporal Detail File ==========\n");
    write_temporal_detail_file(&state);

    // マッチデータ（X, T）を出力
    printf("\n========== Writing Matched Data File ==========\n");
    write_matched_data_file(&state);
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    {
        printf("==============================================\n");
        printf("  GNMiner Phase 2 - Rule Discovery\n");
        printf("==============================================\n\n");
