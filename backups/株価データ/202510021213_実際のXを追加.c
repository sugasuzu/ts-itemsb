...
/* ================================================================================
   ...
   3. 時間変数Tの統合と時系列分析機能の追加
   4. 時間パターンと周期性の検出
   5. 可視化用データ出力
   6. actual_X（実際のX値）の記録と出力（新規追加）
   ================================================================================
*/
...
struct temporal_rule
{
    ...
    double x_mean;
    double x_sigma;
    double actual_x_mean;  /* 実際のX値の平均（新規追加） */
    double actual_x_sigma; /* 実際のX値の標準偏差（新規追加） */
    int support_count;
    int negative_count;
...
struct time_info *time_info_array = NULL;
...
double ***actual_x_sum = NULL;         /* 実際のX値の合計（新規追加） */
double ***actual_x_sigma_array = NULL; /* 実際のX値の二乗和（新規追加） */

/* 時間パターン追跡配列 */
int ****matched_time_indices = NULL;
int ***matched_time_count = NULL;
...
void allocate_dynamic_memory()
{
    ...
    actual_x_sum = (double ***)malloc(Nkotai * sizeof(double **));         /* 新規追加 */
    actual_x_sigma_array = (double ***)malloc(Nkotai * sizeof(double **)); /* 新規追加 */
    ...
    for (i = 0; i < Nkotai; i++)
    {
        ...
        actual_x_sum[i] = (double **)malloc(Npn * sizeof(double *));         /* 新規追加 */
        actual_x_sigma_array[i] = (double **)malloc(Npn * sizeof(double *)); /* 新規追加 */
        ...
        for (j = 0; j < Npn; j++)
        {
            ...
            actual_x_sum[i][j] = (double *)malloc(MAX_DEPTH * sizeof(double));         /* 新規追加 */
            actual_x_sigma_array[i][j] = (double *)malloc(MAX_DEPTH * sizeof(double)); /* 新規追加 */
    ...
    if (match_count != NULL)
    {
        for (i = 0; i < Nkotai; i++)
        {
            for (j = 0; j < Npn; j++)
            {
                ...
                free(actual_x_sum[i][j]);         /* 新規追加 */
                free(actual_x_sigma_array[i][j]); /* 新規追加 */
            ...
            free(actual_x_sum[i]);         /* 新規追加 */
            free(actual_x_sigma_array[i]); /* 新規追加 */
        ...
        free(x_sum);
        free(x_sigma_array);
        free(actual_x_sum);         /* 新規追加 */
        free(actual_x_sigma_array); /* 新規追加 */
...
void initialize_rule_pool()
{
    ...
    for (i = 0; i < Nrulemax; i++)
    {
        ...
        rule_pool[i].actual_x_mean = 0;  /* 新規追加 */
        rule_pool[i].actual_x_sigma = 0; /* 新規追加 */
    ...
    for (individual = 0; individual < Nkotai; individual++)
    {
        ...
        for (k = 0; k < Npn; k++)
        {
            for (i = 0; i < MAX_DEPTH; i++)
            {
                ...
                actual_x_sum[individual][k][i] = 0;         /* 新規追加 */
                actual_x_sigma_array[individual][k][i] = 0; /* 新規追加 */
...
void register_new_rule(struct trial_state *state, int *rule_candidate, int *time_delays,
                       ...
                       double actual_x_mean, double actual_x_sigma, /* 新規追加 */
                       ...
                       int num_attributes, int individual, int k, int depth)
{
    ...
    rule_pool[idx].actual_x_mean = actual_x_mean;   /* 新規追加 */
    rule_pool[idx].actual_x_sigma = actual_x_sigma; /* 新規追加 */
...
void extract_rules_from_individual(struct trial_state *state, int individual)
{
    ...
    for (k = 0; k < Npn; k++)
    {
        ...
        for (loop_j = MIN_ATTRIBUTES; loop_j < 9; loop_j++)
        {
            ...
            double actual_sigma_x = actual_x_sigma_array[individual][k][loop_j]; /* 新規追加 */
                ...
                if (j2 < 9 && j2 >= MIN_ATTRIBUTES)
                {
                    if (!check_rule_duplication(rule_candidate, state->rule_count))
                    {
                        register_new_rule(state, rule_candidate, time_delay_memo,
                                          ...
                                          actual_x_sum[individual][k][loop_j], actual_sigma_x, /* 新規追加 */
...
void export_rule_timeseries(int rule_idx, int trial_id)
{
    ...
    for (i = 0; i < Nrd; i++)
    {
        ...
        fprintf(file, "%s,%.3f,%.3f,%.3f,%.3f,%.3f,%d,%d,%d,%d\n",
                ...
                rule_pool[rule_idx].actual_x_mean,  /* 新規追加 */
                rule_pool[rule_idx].actual_x_sigma, /* 新規追加 */
                matched,
...
void create_trial_files(struct trial_state *state)
{
    ...
    {
        fprintf(file, "Attr1\tAttr2\tAttr3\tAttr4\tAttr5\tAttr6\tAttr7\tAttr8\t");
        fprintf(file, "Actual_X_mean\tActual_X_sigma\t"); /* 新規追加 */
...
void write_global_pool(struct trial_state *state)
{
    ...
    if (file_a != NULL)
    {
        ...
        fprintf(file_a, "Actual_X_mean\tActual_X_sigma\t"); /* 新規追加 */
    ...
    if (file_b != NULL)
    {
        fprintf(file_b, "# Rule Pool Summary with Temporal Patterns and Actual X Values\n");
...
void print_final_statistics()
{
    ...
    printf("(with actual_X value tracking)\n");
    printf("========================================\n");
    printf("Time-Series Configuration:\n");
    printf("  Mode: Phase 2 (Temporal Pattern Analysis + Actual X)\n");
    printf("  Adaptive learning: %s\n", ADAPTIVE_DELAY ? "Enabled" : "Disabled");
...
int main()
{
    ...
    for (trial = Nstart; trial < (Nstart + Ntry); trial++)
    {
        ...
        {
            printf("  [Time-Series Mode Phase 2: Temporal Analysis with actual_X]\n");
            printf("  [Minimum attributes: %d]\n", MIN_ATTRIBUTES);
        }
