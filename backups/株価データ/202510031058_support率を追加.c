...
#define MAX_TIME_DELAY_PHASE2 3 // Phase 2での最大時間遅延（t-3まで参照可能）
...
/* ファイル名
   入力データと出力ファイルのパスを定義 */
#define DATANAME "nikkei225_data/gnminer_individual/7203.txt" // 入力データファイル（銘柄7203）
...
void write_local_output(struct trial_state *state)
{
    ...
    for (i = 1; i < state->rule_count; i++)
    {
        ...
        // 予測値の統計
        fprintf(file, "%8.3f\t%5.3f\t%d\t%6.4f\t%2d\t%2d\t%d\t",
                rule_pool[i].x_mean,
                rule_pool[i].x_sigma,
...
void write_global_pool(struct trial_state *state)
{
    ...
    if (file_a != NULL)
    {
        ...
        for (i = 1; i < state->rule_count; i++)
        {
            ...
            // 予測値の統計
            fprintf(file_a, "%8.3f\t%5.3f\t%d\t%6.4f\t%d\t%d\t%d\t%d\t",
...
int main()
{
    ...
    // この時点でNrdとNzkが設定され、動的メモリが割り当てられる
