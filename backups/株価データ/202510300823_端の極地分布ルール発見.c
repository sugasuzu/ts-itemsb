...
void calculate_and_write_extreme_scores()
{
    ...
    // CSVヘッダーを出力（新指標追加）
    fprintf(file, "rule_idx,signal_strength,SNR,extremeness,t_statistic,p_value,extreme_rate,extreme_signal_score,matched_count,tail_event_rate,directional_bias,non_zero_rate,positive_rate,very_extreme_rate,mean_abs_ratio,spread_ratio,p75_abs,p90_abs,p95_abs\n");
    ...
    for (i = 0; i < global_rule_count; i++)
    {
        ...
        // CSVに1行出力（新指標を全て出力）
        fprintf(file, "%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
                i,
                signal_strength,
                SNR,
