...
#define MIN_SUPPORT_COUNT 10 // 最小サポートカウント（2回以上のマッチが必要、統計的信頼性のため）

/* 方向性バイアス評価パラメータ
   正または負の方向に明確な偏りを持つルールを発見するための閾値 */
#define ENABLE_DIRECTIONAL_BIAS 1     // 方向性バイアス評価の有効化（1=有効, 0=無効）
#define MIN_ABS_MEAN 0.1              // 最小絶対平均値（0.1%以上の偏りが必要）
#define MIN_WIN_RATE_POSITIVE 0.52    // 正バイアス: 勝率52%以上（上昇優位）
#define MAX_WIN_RATE_NEGATIVE 0.48    // 負バイアス: 勝率48%以下（下落優位）
#define MIN_PROFIT_FACTOR 1.1         // 正バイアス: 損益比1.1以上
#define MAX_PROFIT_FACTOR 0.9         // 負バイアス: 損益比0.9以下
#define DIRECTIONAL_RELAXED_SIGMA 2.0 // 方向性バイアス時の最大σ（標準の0.5より大幅に緩和）
...
struct temporal_rule
{
    ...
    double future_mean[FUTURE_SPAN];  // 各未来時点の予測値平均（全体平均、後方互換性のため維持）
    double future_sigma[FUTURE_SPAN]; // 各未来時点の予測値標準偏差（全体標準偏差、後方互換性のため維持）

    // 【新規】方向性分離統計（t+1, t+2, ..., t+FUTURE_SPAN）
    double positive_mean[FUTURE_SPAN];   // X > 0 の平均（上昇時の期待値）
    double negative_mean[FUTURE_SPAN];   // X < 0 の平均（下落時の期待値）
    int positive_count[FUTURE_SPAN];     // X > 0 の件数（上昇回数）
    int negative_count_dir[FUTURE_SPAN]; // X < 0 の件数（下落回数）※既存のnegative_countと区別
    double win_rate[FUTURE_SPAN];        // 勝率 = positive_count / (positive_count + negative_count_dir)
    double profit_factor[FUTURE_SPAN];   // 損益比 = sum(X>0) / |sum(X<0)|
    double sharpe_ratio[FUTURE_SPAN];    // シャープレシオ = |mean| / sigma（方向性の強さ）
...
void initialize_rule_pool()
{
    ...
    for (i = 0; i < Nrulemax; i++)
    {
        ...
        for (int k = 0; k < FUTURE_SPAN; k++)
        {
            ...
            /* 【新規】方向性分離統計の初期化 */
            rule_pool[i].positive_mean[k] = 0.0;
            rule_pool[i].negative_mean[k] = 0.0;
            rule_pool[i].positive_count[k] = 0;
            rule_pool[i].negative_count_dir[k] = 0;
            rule_pool[i].win_rate[k] = 0.0;
            rule_pool[i].profit_factor[k] = 0.0;
        }
 ...
 * @param win_rate_array 勝率配列 [FUTURE_SPAN]
 * @param profit_factor_array 損益比配列 [FUTURE_SPAN]
 * @param support サポート値
...
int check_rule_quality(double *future_sigma_array, double *future_mean_array,
                       double *win_rate_array, double *profit_factor_array,
                       double support, int num_attributes, int support_count)
{
    ...
    // 方向性バイアス評価：正または負の方向に明確な偏りを持つルールを発見
    // いずれかの時点で方向性バイアスがあれば、σの制約を緩和
    for (i = 0; i < FUTURE_SPAN; i++)
    {
        double mean = future_mean_array[i];
        double sigma = future_sigma_array[i];
        double win_rate = win_rate_array[i];
        double profit_factor = profit_factor_array[i];
        double abs_mean = fabs(mean);

        // 正の方向に偏った分布（上昇優位）
        if (mean > MIN_ABS_MEAN &&
            win_rate >= MIN_WIN_RATE_POSITIVE &&
            profit_factor >= MIN_PROFIT_FACTOR &&
            sigma <= DIRECTIONAL_RELAXED_SIGMA)
        {
            return 1; // 正バイアスルールとして採用
        }

        // 負の方向に偏った分布（下落優位）
        if (mean < -MIN_ABS_MEAN &&
            win_rate <= MAX_WIN_RATE_NEGATIVE &&
            profit_factor <= MAX_PROFIT_FACTOR &&
            sigma <= DIRECTIONAL_RELAXED_SIGMA)
        {
            return 1; // 負バイアスルールとして採用
        }
    }
#endif
...
void register_new_rule(struct trial_state *state, int *rule_candidate, int *time_delays,
                       ...
                       double *positive_mean, double *negative_mean,
                       int *positive_count, int *negative_count_dir,
                       ...
                       int individual, int k, int depth)
{
    ...
    for (i = 0; i < FUTURE_SPAN; i++)
    {
        rule_pool[idx].positive_mean[i] = positive_mean[i];
        rule_pool[idx].negative_mean[i] = negative_mean[i];
        rule_pool[idx].positive_count[i] = positive_count[i];
        rule_pool[idx].negative_count_dir[i] = negative_count_dir[i];

        // 勝率の計算
        int total_trades = positive_count[i] + negative_count_dir[i];
        if (total_trades > 0)
        {
            rule_pool[idx].win_rate[i] = (double)positive_count[i] / (double)total_trades;
        }
        else
        {
            rule_pool[idx].win_rate[i] = 0.0;
        }

        // プロフィットファクターの計算
        // positive_mean * positive_count = 正方向の合計利益
        // negative_mean * negative_count_dir = 負方向の合計損失
        double total_positive = positive_mean[i] * positive_count[i];
        double total_negative = negative_mean[i] * negative_count_dir[i];

        if (total_negative < 0.0)
        {
            // 損失がある場合（negative_meanは負値のため、total_negativeも負値）
            rule_pool[idx].profit_factor[i] = total_positive / fabs(total_negative);
        }
        else
        {
            // 損失がない場合
            ...
            {
                rule_pool[idx].profit_factor[i] = 999.0; // 損失なしで利益あり→非常に大きな値
            }
            else
            {
                rule_pool[idx].profit_factor[i] = 0.0; // 利益も損失もない
            }
        }

        // シャープレシオの計算
        // Sharpe Ratio = |mean| / sigma（方向性の強さ指標）
        if (future_sigma[i] > 0.0)
        {
            rule_pool[idx].sharpe_ratio[i] = fabs(future_mean[i]) / future_sigma[i];
        }
        else
        {
            rule_pool[idx].sharpe_ratio[i] = 0.0; // σ=0の場合は0
        }
    }
...
void extract_rules_from_individual(struct trial_state *state, int individual)
{
    ...
    for (k = 0; k < Npn; k++)
    {
        ...
        for (loop_j = MIN_ATTRIBUTES; loop_j < 9; loop_j++)
        {
            ...
            // 勝率と損益比を計算（方向性バイアス評価用）
            double win_rate_temp[FUTURE_SPAN];
            double profit_factor_temp[FUTURE_SPAN];
            for (i2 = 0; i2 < FUTURE_SPAN; i2++)
            {
                int total_trades = positive_count_ptr[i2] + negative_count_dir_ptr[i2];
                if (total_trades > 0)
                {
                    win_rate_temp[i2] = (double)positive_count_ptr[i2] / (double)total_trades;
                }
                else
                {
                    win_rate_temp[i2] = 0.0;
                }

                double total_positive = positive_mean_ptr[i2] * positive_count_ptr[i2];
                double total_negative = negative_mean_ptr[i2] * negative_count_dir_ptr[i2];
                if (total_negative < 0.0)
                {
                    profit_factor_temp[i2] = total_positive / fabs(total_negative);
                }
                else if (total_positive > 0.0)
                {
                    profit_factor_temp[i2] = 999.0;
                }
                else
                {
                    profit_factor_temp[i2] = 0.0;
                }
            }

            // ルールの品質チェック（サポートカウント、標準偏差、属性数、方向性バイアスをチェック）
            if (check_rule_quality(future_sigma_ptr, future_mean_ptr, win_rate_temp, profit_factor_temp, support, j2, matched_count))
            {
                if (j2 < 9 && j2 >= MIN_ATTRIBUTES)
                {
                    ...
                    if (!check_rule_duplication(rule_candidate, state->rule_count))
                    {
                        ...
                        register_new_rule(state, rule_candidate, time_delay_memo,
                                          ...
                                          positive_mean_ptr, negative_mean_ptr,
                                          positive_count_ptr, negative_count_dir_ptr,
...
void write_local_output(struct trial_state *state)
{
    ...
    for (i = 1; i < state->rule_count; i++)
    {
        ...
        for (int k = 0; k < FUTURE_SPAN; k++)
        {
            fprintf(file, "%8.3f\t%8.3f\t%d\t%d\t%5.3f\t%6.2f\t",
                    rule_pool[i].positive_mean[k],
                    rule_pool[i].negative_mean[k],
                    rule_pool[i].positive_count[k],
                    rule_pool[i].negative_count_dir[k],
                    rule_pool[i].win_rate[k],
                    rule_pool[i].profit_factor[k]);
        }
...
void write_global_pool(struct trial_state *state)
{
    ...
    if (file_a != NULL)
    {
        ...
        for (int k = 1; k <= FUTURE_SPAN; k++)
        {
            fprintf(file_a, "X(t+%d)_pos_mean\tX(t+%d)_neg_mean\tX(t+%d)_pos_cnt\tX(t+%d)_neg_cnt\tX(t+%d)_win_rate\tX(t+%d)_profit_factor\t",
                    k, k, k, k, k, k);
        }
        ...
        for (i = 0; i < global_rule_count; i++)
        {
            ...
            for (int k = 0; k < FUTURE_SPAN; k++)
            {
                fprintf(file_a, "%8.3f\t%8.3f\t%d\t%d\t%5.3f\t%6.2f\t",
                        global_rule_pool[i].positive_mean[k],
                        global_rule_pool[i].negative_mean[k],
                        global_rule_pool[i].positive_count[k],
                        global_rule_pool[i].negative_count_dir[k],
                        global_rule_pool[i].win_rate[k],
                        global_rule_pool[i].profit_factor[k]);
            }
    ...
    if (file_b != NULL)
    {
        ...
        for (i = 0; i < display_limit; i++)
        {
            ...
            /* 【新規】方向性分離統計を出力（t+1, t+2, ..., t+FUTURE_SPAN）*/
            for (int k = 0; k < FUTURE_SPAN; k++)
            {
                fprintf(file_b, "      Directional(t+%d): pos_mean=%.3f (n=%d), neg_mean=%.3f (n=%d), win_rate=%.1f%%, PF=%.2f\n",
                        k + 1,
                        global_rule_pool[i].positive_mean[k],
                        global_rule_pool[i].positive_count[k],
                        global_rule_pool[i].negative_mean[k],
                        global_rule_pool[i].negative_count_dir[k],
                        global_rule_pool[i].win_rate[k] * 100.0,
                        global_rule_pool[i].profit_factor[k]);
            }
