...
char direction_str[20] = "positive";         // 方向文字列 ("positive" or "negative")
int extreme_direction = 1;                   // 1: 正(positive), -1: 負(negative)
        ...
        for (k = 0; k < Npn; k++)
        {
            for (j = 1; j < 9; j++)
            {
                if (match_count[individual][k][j] != 0)
                {
                    ...
                    t_sum_julian[individual][k][j] /= (double)n;

                    // T（時間）の標準偏差を計算（不偏標準偏差）
                    if (n > 1)
                    {
                        // 標本分散を計算
                        double variance = t_sigma_julian_array[individual][k][j] / (double)n -
                        ...
                        t_sigma_julian_array[individual][k][j] = sqrt(variance);
                    }
                    else
                    {
                        // サンプル数が1の場合、標準偏差は計算不可
...
void calculate_and_write_extreme_scores()
{
    ...
    // CSVヘッダーを出力（方向性情報追加）
    fprintf(file, "rule_idx,dataset_type,extremeness,t_statistic,p_value,extreme_rate,extreme_signal_score,matched_count,tail_event_rate,directional_bias,non_zero_rate,positive_rate,very_extreme_rate,mean_abs_ratio,spread_ratio,p75_abs,p90_abs,p95_abs\n");
...
void write_global_pool(struct trial_state *state)
{
    ...
    if (file_a != NULL)
    {
        ...
        fprintf(file_a, "Extremeness\tt_stat\tp_value\tTailEventRate\tExtremeScore\n");
 ...
 */
void setup_paths_for_forex(const char *code, const char *direction)
{
    // 通貨ペアコードをコピー
    ...
    // 方向文字列をコピー
    strncpy(direction_str, direction, sizeof(direction_str) - 1);
    ...
    // データファイルパスを設定（方向別ファイル）
    snprintf(data_file_path, sizeof(data_file_path),
             "forex_data/extreme_gnminer/%s_%s.txt", forex_pair, direction_str);

    // 出力ベースディレクトリを設定（通貨ペア/方向 の階層構造）
    snprintf(output_base_dir, sizeof(output_base_dir),
             "output/%s/%s", forex_pair, direction_str);
    ...
    printf("Direction:  %s (%s)\n", direction_str,
           extreme_direction == 1 ? "BUY signal" : "SELL signal");
    printf("Input File: %s\n", data_file_path);
    printf("Output Dir: %s\n", output_base_dir);
    printf("=========================\n\n");
}
 ...
 * @return 成功時0、失敗時1
 */
int process_single_forex(const char *code, const char *direction)
{
    ...
    // パス設定
    setup_paths_for_forex(code, direction);
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    {
        printf("==============================================\n");
        printf("  GNMiner Phase 2 - Directional Extreme Predictor\n");
        printf("==============================================\n\n");
        printf("Usage: %s <forex_pair> <direction>\n\n", argv[0]);
        printf("Arguments:\n");
        printf("  <forex_pair>  Currency pair code (e.g., USDJPY, EURUSD)\n");
        printf("  <direction>   'positive' for BUY signals (X >= 1.0%%)\n");
        printf("                'negative' for SELL signals (X <= -1.0%%)\n\n");
        printf("Examples:\n");
        printf("  %s USDJPY positive  # Discover BUY rules for USD/JPY\n", argv[0]);
        printf("  %s USDJPY negative  # Discover SELL rules for USD/JPY\n", argv[0]);
        printf("  %s EURUSD positive  # Discover BUY rules for EUR/USD\n", argv[0]);
        printf("  %s GBPJPY negative  # Discover SELL rules for GBP/JPY\n\n", argv[0]);
        printf("Note:\n");
        printf("  - Data files must exist: forex_data/extreme_gnminer/{PAIR}_{direction}.txt\n");
        printf("  - Results will be saved to: output/{PAIR}/{direction}/\n");
    ...
    if (argc < 3)
    {
        ...
        printf("Usage: %s <forex_pair> <direction>\n\n", argv[0]);
        printf("  <forex_pair>: Currency pair (e.g., USDJPY, EURUSD)\n");
        printf("  <direction>:  'positive' or 'negative'\n\n");
        printf("Examples:\n");
        printf("  %s USDJPY positive\n", argv[0]);
        printf("  %s EURJPY negative\n\n", argv[0]);
    ...
    // コマンドライン引数から通貨ペアと方向を取得
    const char *forex_pair_arg = argv[1];
    const char *direction_arg = argv[2];

    // 方向の妥当性チェック
    if (strcmp(direction_arg, "positive") != 0 && strcmp(direction_arg, "negative") != 0)
    {
        printf("\nERROR: Invalid direction '%s'\n\n", direction_arg);
        printf("Direction must be either 'positive' or 'negative'\n\n");
        return 1;
    }
    ...
    printf("\n");
    printf("==================================================\n");
    printf("  GNMiner Phase 2 - Directional Rule Discovery\n");
    printf("==================================================\n");
    printf("Forex Pair: %s\n", forex_pair_arg);
    printf("Direction:  %s (%s signals)\n", direction_arg,
           strcmp(direction_arg, "positive") == 0 ? "BUY" : "SELL");
    printf("==================================================\n");
    printf("\n");

    // 処理実行
    int result = process_single_forex(forex_pair_arg, direction_arg);
