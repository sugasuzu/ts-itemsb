...
#define MIN_ATTRIBUTES 2 // 最小属性数（新規追加：2以上の属性が必要）
...
/* ================================================================================
   ルール抽出系関数（1次元版） - 最小属性数制約追加
   ================================================================================
*/

int check_rule_quality_1d(double sigma_x, double support, int num_attributes)
{
    /* 最小属性数のチェックを追加 */
...
void extract_rules_from_individual(struct trial_state *state, int individual)
{
    ...
    for (k = 0; k < Npn; k++)
    {
        ...
        for (loop_j = MIN_ATTRIBUTES; loop_j < 9; loop_j++)
        {
            ...
            /* 最小属性数チェックを含む品質チェック */
            if (check_rule_quality_1d(sigma_x, support, j2))
            {
