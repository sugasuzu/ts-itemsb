 ...
 */
int check_rule_quality(double *future_sigma_array, double support, int num_attributes)
{
    int i;
...
void extract_rules_from_individual(struct trial_state *state, int individual)
{
    ...
    for (k = 0; k < Npn; k++)
    {
        ...
        for (loop_j = MIN_ATTRIBUTES; loop_j < 9; loop_j++)
        {
            ...
            // ルールの品質チェック（t+1, t+2, t+3全ての時点の標準偏差をチェック）
            if (check_rule_quality(future_sigma_ptr, support, j2))
            {
