...
#define fpoola "zrp01a.txt"       /* 出力: グローバルルールプール（属性名版） \
                                     人間が読みやすい形式でルールを記録 */
#define fpoolb "zrp01b.txt"       /* 出力: グローバルルールプール（数値版） \
                                     プログラム処理用の数値形式 */
#define fcont "zrd01.txt"         /* 出力: 実行統計ドキュメント \
                                     各試行でのルール発見数等を記録 */
#define RESULTNAME "zrmemo01.txt" /* 出力: デバッグ用メモファイル（現在未使用） */
...
void main()
{
    ...
    for (trial_index = Nstart; trial_index < (Nstart + Ntry); trial_index++)
    {
        ...
        /* ================================================================================
           ファイル名生成処理
           試行番号から5桁の数字を抽出し、ユニークなファイル名を作成
           例: trial_index=12345 → IL12345.txt, IA12345.txt, ...
           ================================================================================ */
        ...
        /* ルールファイル名生成: IL#####.txt */
        filename_rule[0] = 73;               /* ASCII 73 = 'I' : Internalの意 */
        filename_rule[1] = 76;               /* ASCII 76 = 'L' : Listの意（ルールリスト） */
        filename_rule[2] = 48 + file_digit5; /* 48='0'のASCII、数字に変換 */
        filename_rule[3] = 48 + file_digit4;
        filename_rule[4] = 48 + file_digit3;
        filename_rule[5] = 48 + file_digit2;
        filename_rule[6] = 48 + file_digit1;
        filename_rule[7] = 46;   /* ASCII 46 = '.' */
        filename_rule[8] = 116;  /* ASCII 116 = 't' */
        filename_rule[9] = 120;  /* ASCII 120 = 'x' */
        filename_rule[10] = 116; /* ASCII 116 = 't' */
        filename_rule[11] = 0;   /* NULL終端: C文字列の終了マーク */

        /* レポートファイル名生成: IA#####.txt */
        filename_report[0] = 73; /* 'I' */
        filename_report[1] = 65; /* ASCII 65 = 'A' : Analysisの意 */
        ...
        filename_report[8] = 116;  /* 't' */
        filename_report[9] = 120;  /* 'x' */
        filename_report[10] = 116; /* 't' */
        filename_report[11] = 0;

        /* 一時ファイル名生成: IT#####.txt */
        filename_temp[0] = 73; /* 'I' */
        filename_temp[1] = 84; /* ASCII 84 = 'T' : Temporaryの意 */
        ...
        filename_local[1] = 66; /* ASCII 66 = 'B' : Backupの意 */
