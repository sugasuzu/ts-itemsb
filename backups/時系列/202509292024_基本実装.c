...
/* ================================================================================
   ...
   時系列対応版GNMiner - Phase 1実装

   【主要な変更点】
   - 各判定ノードに時間遅延フィールドを追加
   - 固定遅延（全て1時刻前）でのプロトタイプ実装
   - 予測時点は1時刻後（t+1）
   - 出力に時間遅延情報を表示

   【時系列ルール形式】
   A1(t-d1) ∧ A2(t-d2) ∧ ... → (X,Y)(t+1)
   where di は各属性の時間遅延（Phase 1では全て1）
   ================================================================================ */
...
#define MAX_TIME_DELAY 10    /* 最大時間遅延（将来の拡張用） */
#define DEFAULT_TIME_DELAY 1 /* Phase 1での固定遅延値 */
...
#define Muratej 6
#define Muratea 6
#define Muratedelay 6 /* 時間遅延の突然変異率（Phase 2以降で使用） */
#define Nkousa 20
...
int main()
{
    ...
    for (trial_index = Nstart; trial_index < (Nstart + Ntry); trial_index++)
    {
        ...
        if (TIMESERIES_MODE)
        {
            printf("  [Time-Series Mode: Fixed delay = %d]\n", DEFAULT_TIME_DELAY);
        }
        ...
        for (i4 = 0; i4 < Nkotai; i4++)
        {
            for (j4 = 0; j4 < (Npn + Njg); j4++)
            {
                ...
                /* ★時間遅延の初期化（Phase 1: 全て固定値） */
                gene_time_delay[i4][j4] = DEFAULT_TIME_DELAY;

                /* Phase 2以降のランダム初期化（コメントアウト）
                gene_time_delay[i4][j4] = rand() % (MAX_TIME_DELAY + 1);
                */
            }
        }
        ...
        for (generation_id = 0; generation_id < Generation; generation_id++)
        {
            ...
            for (individual_id = 0; individual_id < Nkotai; individual_id++)
            {
                ...
                for (k = 0; k < Npn; k++)
                {
                    for (i = 0; i < 10; i++)
                    {
                        ...
                        time_delay_chain[individual_id][k][i] = 0; /* ★時間遅延連鎖も初期化 */
            ...
            if (TIMESERIES_MODE)
            {
                safe_start_index = DEFAULT_TIME_DELAY;  /* 過去参照のマージン */
            ...
            for (i = 0; i < Nrd; i++)
            {
                ...
                if (TIMESERIES_MODE)
                {
                    if (i < safe_start_index || i >= safe_end_index)
                    {
                        continue;
                    }
                }
            ...
            for (individual_id = 0; individual_id < Nkotai; individual_id++)
            {
                ...
                for (k = 0; k < Npn; k++)
                {
                    ...
                    for (i = 0; i < 10; i++)
                    {
                        ...
                        rule_candidate[i] = 0;
                        rule_memo[i] = 0;
                        time_delay_candidate[i] = 0; /* ★時間遅延候補も初期化 */
                    }
                            ...
                            support_value >= Minsup)
                        {
                            ...
                            if (j2 < 9)
                            {
                                ...
                                if (new_flag == 0)
                                {
                                    ...
                                    /* ★時間遅延も保存（Phase 1では全て固定値） */
                                    rule_pool[rule_count].time_delay0 = DEFAULT_TIME_DELAY;
                                    rule_pool[rule_count].time_delay1 = DEFAULT_TIME_DELAY;
                                    rule_pool[rule_count].time_delay2 = DEFAULT_TIME_DELAY;
                                    rule_pool[rule_count].time_delay3 = DEFAULT_TIME_DELAY;
                                    rule_pool[rule_count].time_delay4 = DEFAULT_TIME_DELAY;
                                    rule_pool[rule_count].time_delay5 = DEFAULT_TIME_DELAY;
                                    rule_pool[rule_count].time_delay6 = DEFAULT_TIME_DELAY;
                                    rule_pool[rule_count].time_delay7 = DEFAULT_TIME_DELAY;
            ...
            for (i4 = 0; i4 < Nkotai; i4++)
            {
                if (fitness_ranking[i4] < 40)
                {
                    for (j4 = 0; j4 < (Npn + Njg); j4++)
                    {
                        ...
                        gene_time_delay[fitness_ranking[i4]][j4] = node_structure[i4][j4][2]; /* ★時間遅延も継承 */
        ...
        /* ★時系列情報付きデバッグ出力（オプション） */
        if (TIMESERIES_MODE && (generation_id % 50 == 0))
        {
            printf("  [Time-Series Debug] All rules use delay t-%d\n", DEFAULT_TIME_DELAY);
        }
    ...
    printf("  Mode: %s\n", TIMESERIES_MODE ? "Enabled" : "Disabled");
    printf("  Fixed delay: t-%d\n", DEFAULT_TIME_DELAY);
    printf("  Prediction span: t+%d\n", PREDICTION_SPAN);
    printf("========================================\n");
