...
/* ================================================================================
   ...
   where di ∈ {0,1,2,3} は進化的に最適化される遅延
   ================================================================================ */
...
#define MAX_TIME_DELAY_PHASE3 3 /* Phase 3での最大遅延値 */
#define MIN_TIME_DELAY 0        /* 最小時間遅延（t-0 = 現在） */
#define PREDICTION_SPAN 1       /* 予測時点（t+1） */
#define ADAPTIVE_DELAY 1        /* 適応的遅延選択の有効化 */
...
#define Muratej 6
#define Muratea 6
#define Muratedelay 6 /* 時間遅延の突然変異率 */
#define Nkousa 20
...
int main()
{
    ...
    /* ★Phase 3追加: 遅延値選択用変数 */
    int mutation_delay1, mutation_delay2, mutation_delay3;
    int total_delay_usage;
    int selected_delay_value;
    ...
    double future_x_value, future_y_value;
    ...
    /* 遅延パターン成功率（各遅延値がどれだけ良いルールを生成したか） */
    ...
    for (trial_index = Nstart; trial_index < (Nstart + Ntry); trial_index++)
    {
        ...
        for (i = 0; i <= MAX_TIME_DELAY_PHASE3; i++)
        {
            ...
            for (j = 0; j < 5; j++)
            {
                delay_usage_history[j][i] = 1; /* 全遅延値に等しいチャンス */
            }
        }
        total_delay_usage = MAX_TIME_DELAY_PHASE3 + 1;
        ...
        for (generation_id = 0; generation_id < Generation; generation_id++)
        {
            ...
            if (TIMESERIES_MODE)
            {
                safe_start_index = MAX_TIME_DELAY_PHASE3;
                safe_end_index = Nrd - PREDICTION_SPAN;
            }
            else
            {
                safe_start_index = 0;
                        ...
                        while (current_node_id > (Npn - 1) && j < Nmx)
                        {
                            ...
                            if (data_buffer[data_index][node_structure[individual_id][current_node_id][0]] == 1)
                            {
                                if (match_flag == 1)
                                {
                                    ...
                                    /* 未来時点(i+PREDICTION_SPAN)のX,Y値を使用 */
            ...
            for (i = 0; i <= MAX_TIME_DELAY_PHASE3; i++)
            {
                ...
                /* 現世代をリセット */
                delay_usage_history[0][i] = 1; /* 最小値を保証 */
